...
export default function App(){
  ...
  useEffect(()=>{
    ...
    setRvrs(lsGet("cbpms-rvrs",REVIEWERS_DEFAULT));
    setAssign(lsGet("cbpms-assign",{}));
    setCreds(lsGet("cbpms-creds",DEFAULT_CREDS));
    setLoad(false);
  },[]);

  const saveD=v=>{setD(v);lsSet("cbpms-d",v);};
  const saveE=v=>{setE(v);lsSet("cbpms-e",v);};
  const saveRevs=v=>{setRevs(v);lsSet("cbpms-revs",v);};
  const saveRvrs=v=>{setRvrs(v);lsSet("cbpms-rvrs",v);};
  const saveAssign=v=>{setAssign(v);lsSet("cbpms-assign",v);};
  const saveCreds=v=>{setCreds(v);lsSet("cbpms-creds",v);};
  ...
  if(loading)return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,fontFamily:"Inter,sans-serif"}}>
      ...
      <div style={{color:C.text4,fontSize:13}}>Loading…</div>
    </div>
  );
